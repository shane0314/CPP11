...
struct animal
{
  std::string name;
  int legs;
  friend std::ostream& operator<<(std::ostream& os,const animal& e);
};

std::ostream& operator<<( std::ostream& os,const animal& e)
{
    os << e.name << " " << e.legs;
}
...
int main()
{
  ...
  std::cout << rand_index.size() << '\n';
  std::cout << rand_index[0].name << '\n';
  std::cout << rand_index[1].name << '\n';
  std::cout << rand_index[2].name << '\n';
  
  // sequenced_index: list elements
